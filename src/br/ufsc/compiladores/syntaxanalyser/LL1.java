package br.ufsc.compiladores.syntaxanalyser;

import br.ufsc.compiladores.common.Symbol;
import br.ufsc.compiladores.grammar.cfg.Body;
import br.ufsc.compiladores.grammar.cfg.ContextFreeGrammar;
import br.ufsc.compiladores.lexicalanalyser.LexicalAnalyser;
import br.ufsc.compiladores.lexicalanalyser.Token;
import br.ufsc.compiladores.lexicalanalyser.TokenType;
        ...
        while(!stack.isEmpty()) {
            ...
            if (!top.isVariable() && symbol.equals(top)) {
                ...
                try {
                    ...
                    errors.add(String.format("Erro sintático na linha (%s), palavra (%s): lexema com problema: %s", token.getRowPos(), token.getWordPos(), token.getName()));
                    return;
            }
        }}
    }
        ...
        for (var key : productions.keySet()) {
            ...
            for (var body : productions.get(key)) {
                ...
                for (var symbol : symbols) {
                    if (symbol.equals(Symbol.CFG_EPSILON)) {
                        this.follows.get(key).forEach(
                                s -> this.table.get(key).put(s, eBody)
                        );
                    } else {
